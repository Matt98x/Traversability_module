...
vector<float> transform_lc; // transformation from body to camera
...
{ 
	...
	if(is_robot){
        ...
        tf::StampedTransform out1;
        tf::StampedTransform out2;
        vector<tf::StampedTransform> Out;
        ...
        Out.push_back(tf::StampedTransform(transform, ros::Time::now(), "trunk", "world"));
		...
		tf::quaternionMsgToTF (output.state[0].pose.pose.orientation , q);
		transform.setRotation(q); 
        Out.push_back(tf::StampedTransform(transform, ros::Time::now(),camera_frame, "trunk"));
...
int main (int argc, char **argv)
{
  ...
  ros::NodeHandle n;
	...
	n.getParam("/pointcloud_frame_id",lidar_frame);
    n.getParam("/transform_body_marker",transform_tc);
  ...
  message_filters::Subscriber<geometry_msgs::PoseStamped> odom1(n, body_state, 1);
